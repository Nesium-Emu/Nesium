...
impl RingBuffer {
    fn new(capacity: usize, output_sample_rate: u32) -> Self {
        ...
        log::info!("Audio initialized: APU rate {}Hz -> Output rate {}Hz (ratio: {:.6})", 
            APU_SAMPLE_RATE as u32, output_sample_rate, resample_ratio);
...
impl AudioOutput {
    ...
    pub fn new() -> Option<Self> {
        ...
        log::info!("Audio config: channels={}, sample_rate={}", 
            config.channels, output_sample_rate);
            ...
            cpal::SampleFormat::I16 => {
                ...
                device.build_output_stream(
                    ...
                    move |data: &mut [i16], _: &cpal::OutputCallbackInfo| {
                        if let Ok(mut buffer) = buffer_clone.lock() {
                            let sample_count = if channels == 2 { data.len() / 2 } else { data.len() };
